...
func InitializeApp(redisHost config.RedisHostType, redisPort config.RedisPortType, redisPassword config.RedisPasswordType, redisDB int, coinGeckoAPIURL config.CompareFinAPIURLType, coinGeckoAPIKeyType config.CompareFinAPIKeyType) (*App, func(), error) {
...
var RedisSet = wire.NewSet(redis.NewRedisClient, redis.NewCurrencyRepository, wire.Bind(new(domain.CurrencyRepository), new(*redis.CurrencyRepository)))
